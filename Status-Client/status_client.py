...
def do_auth():
    dic = json.dumps({"username": USER,
                      "password": PASSWORD,
                      }).encode('utf-8')
    length = len(dic).to_bytes(4, byteorder='big')
    magic_num_type = bytearray('bubble'.encode('ascii'))
    magic_num_type.append(0x0)
    bytes_sent = magic_num_type + length + dic
    s.send(bytes_sent)
    print(bytes_sent)
...
def sent_message(message):
    ...
    length = len(content).to_bytes(4, byteorder='big')
    magic_num_type = bytearray('bubble'.encode('ascii'))
    magic_num_type.append(0x1)
    bytes_sent = magic_num_type + length + content
    s.send(bytes_sent)
    ...
    while True:
        try:
            ...
            while True:
                ...
                # 丢包率
                array['loss_10010'] = lostRate.get('10010') * 100
                array['loss_189'] = lostRate.get('189') * 100
                array['loss_10086'] = lostRate.get('10086') * 100
                # 时延
                array['ping_10010'] = pingTime.get('10010')
                array['ping_189'] = pingTime.get('189')
                array['ping_10086'] = pingTime.get('10086')
                # 连接、进程、线程数量
                array['tcp'], array['udp'], array['process'], array['thread'] = tupd()
                array['io_read'] = diskIO.get("read")
                array['io_write'] = diskIO.get("write")

                sent_message(json.dumps(array))
                time.sleep(1)
