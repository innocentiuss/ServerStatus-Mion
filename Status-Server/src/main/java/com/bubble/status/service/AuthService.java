...
public class AuthService {
    ...
    Map<String, ServerOnlineInfo> authed = new ConcurrentHashMap<>();
    ...
    public boolean doAuth(String connectInfos, String ip, int port) {
        AuthInfo authInfo = JsonUtil.toObject(connectInfos, AuthInfo.class);
        ServerOnlineInfo savedInfo = configService.getInfoFromUsername(authInfo.getUsername());
        // 认证失败
    ...
    public void disConnect(String ip, int port) {
        String key = ip + port;
        ServerOnlineInfo info = authed.get(key);
        if (info != null) {
